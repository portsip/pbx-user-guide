...
您还可通过 [PortSIP Forum](https://forum.portsip.com/) 获得技术支持。
...
免费版的 PortSIP PBX 最大支持 3 路并发通话，无用户（分机）数量限制。如果您需要更多的并发通话数量支持，请参阅 [11.9 许可证](#11.9 许可证) 一节。
...
+ 如果你要安装 PBX的 Linux 电脑/服务器位于局域网里，必须分配一个静态的局域网 IP 地 址；如果位于公网，必须得有一个公网静态 IP。 
...
+ UDP 端口：**45000– 65000, 33000 -  34999**。这些端口用于 RTP 传输媒体数据。
+ TCP 端口：**8899– 8900, 8881, 8882, 8883, 8884, 8887, 8888,9333、10443**。这些端口用于服务器控制和 WebRTC Gateway 传输。
...
在成功安装了 PortSIP PBX 之后，必须设置 Linux 防火墙规则，否则 PBX 将无法正常运行。 

如果防火墙拦截了相关端口，必须打开如下端口以使得 PortSIP PBX 正常运行。 

+ UDP 端口：**45000– 65000、33000 -  34999**。这些端口用于 RTP 传输媒体数据，包括音视频包。 
+ TCP：**8899– 8900、8881、8882、8883、8884、8887、8888、9333、10443**。这些端口主要用于服务器控制和 WebRTC Gateway 传输。
+ UDP：**5060**。这是默认的 UDP 端口，用于传输 SIP 消息，发送和接收 SIP 信令。 

如需在 PBX 里面增加新的 SIP 传输协议，还需要同时在防火墙里面打开对应的端口：
...
**重要提示：**如果您是在 AWS 等云平台上运行 PBX，并且云平台具有自己的防火墙，您还必须在云平台防 火墙上打开这些端口。 



### 2.4 从 v12.0/v12.1 升级

> **警告：**警告：由于 v12.2 里的 trunk 和接入规则相关设置已经改变，请注意如下几点：
>
> - 如果在 v12.0/v12.1 里给某条接入规则分配了多个 trunk/provider，则升级到v12.2之后，只会为这条规则保留一个 trunk/provider
>
> - 升级之前，必须检查所有租户的所有接入规则。如果接入规则的 DID 号码掩码包含有范围、多个号码或者通配符 *，那么必须对接入规则的 DID 号码掩码进行修改，因为在 v12.2里面，接入规则只允许单个 * 或者单个具体的号码
>
>   例如，在 v12.1 里，接入规则的 DID 号码掩码是 123;210-220;56*; *89，那么在升级之前必须将其修改为单个号码，比如123、210，或者456，否则升级会失败。
>



**因此，我们建议您在升级到 v12.2之前对您当前的 PBX 进行备份，并下载备份文件保存在其他地方。**

备份步骤请见第 [11.8 小节](#11.8 备份和还原)



#### Windows 升级

在下载完成 v12.2 安装包之后，您只需双击安装包程序即可按照指引进行升级。
...
> + 如果您想 PBX 容器在您每次重启 Linux 都自动启动，可以在下属命令中加入 **--restart=always** 参数.
...
### 2.5 消除 HTTPS 证书安全警告信息

PortSIP PBX 在 8888 端口监听并提供 HTTP 服务，以便访问 PBX 管理控制台。

假定将 PortSIP PBX 安装在 IP 地址为 172.217.14.16 的服务器上，那么您可以使用 http://172.217.14.16:8888 来访问 PortSIP PBX的WEB 管理界面。注意：推荐使用 Chrome 和 Firefox 浏览器，请勿使用 IE 浏览器。
...
要避免 SSL 证书警告信息，您需要购买签名的证书（由可信任的证书机构发放的授权证书），替 换自签名证书。要执行该操作，请遵循以下步骤： 
...
+ 登录到 https://yourpbx.com:8887，访问 PortSIP PBX 管理控制台。



注意：您还可以从 [Let’s Encrypt](https://letsencrypt.org/) 免费获取 SSL 证书。
...
- 在浏览器中打开 http://pbxserverip:8888，访问 PortSIP PBX 管理控制台（请勿使用 IE 浏览器）。
...
PortSIP PBX 可部署在 AWS、GCE、AZURE、Digital Ocean、阿里云、UCloud、腾讯云等主流云平台。
...
- 如果没有从第三方证书机构购买证书，那么你需要生成自签名证书。请从博瞻信息的网站下载证书生成工具并运行（或者运行 PBX 安装目录下的 *PortCertMaker.exe*），输入公司名称（英文或者拼音），以及 PBX 的 SIP 域名，然后点击“**生成**”按钮，证书工具将生成自签名证书。

- 生成的证书包括如下三个文件（假设您的 SIP 域名是 portsip.io）：
  domain_key_portsip.io.pem
  domain_cert_portsip.io.pem
  root_cert_portsip.io.pem

  如果你需要从第三方证书证书提供商处购买证书，那么请按照如下步骤（假设为 portsip.io 购买证书）： 

  - 根据证书提供商的指示生成 CSR 文件和私有证书文件并保存。如果你在生成私有证书 的时候设置了密码，请记录下来。
  - 将私有证书文件重命名为 domain_key_portsip.io.pem
  - 将 CSR 提交给证书提供商，在你的证书申请被通过之后，请从证书提供商处下载你的证书文件，证书文件通常有两个：Intermediate CA 证书 和 SSL 证书。
  - 用一个纯文本编辑器（不能用WORD）分别打开 Intermediate CA 证书 和 SSL 证书， 然后将 Intermediate CA 证书的全部内容复制后粘贴在 SSL 证书内容的后面，将合并后 的 SSL 证书文件另存为 *domain_cert_portsip.io.pem*。
  - 从你的证书提供商处下载根证书文件并保存为 *root_cert_portsip.io.pem*。

- 点击“**添加**”按钮，然后在“**传输协议**”窗口，在协议下拉框里选择 TLS 或者 WSS， TLS/WSS 传输协议的默认端口分别为 5063/5065。您也可以使用其他的端口，需要确保您所使用的端口没有被其他程序所占用。 

- 点击证书文件后面的“**上传**”按钮选择域名证书文件，即以 domain_cert_ 开头的 pem 文件： 为“**证书文件**”选择“*domain_cert_portsip.com.pem*”，为“**私钥文件**”选择 “*domain_key_portsip.io.pem*”，为“**根证书文件**”选择“*root_cert_portsip.io.pem*”。 

- 输入“**私钥文件密码**”，用博瞻信息公司证书工具生成的证书没有密码，这里不需要输入，保留为空；如果您是从第三方证书提供商那里购买的证书，并且在生成私有证书的时候设置了密码，请在这里输入您设置给私有证书的密码，如果没有设置就保留为空。 
...
- DID POOL 允许设置数字前缀和通配符 \*。例如，设置为 44\*\*\*\*\*\*, 则表示所有以 44 开头的八位号码。当租户创建接入规则的时候，所有 44 开头的八位号码都可以作为 DID 号码用于接入规则。
...
- **为来自黑名单号码的呼叫播放特定语音**：选中此选项后，如果主叫方已加入该呼叫队列的黑名单，则您可为其设置特殊提示音。主叫方听到提示音后，如果按下了 DTMF，则 PBX 会继续正常的呼叫队列工作流。
...
您可向呼叫队列添加一个或多个号码。黑名单号码向呼叫队列发起通话后，系统会向他们播放特别提示音。如需了解更多详情，请参考[配置队列选项](#配置队列选项)。
...
PortSIP PBX 内置集成了 WebRTC Gateway，可通过网页桥接 PBX 和 WebRTC 客户端。
...
- **侦听 WS**：选中该选项后，您还需要填写以下字段：

- **WSS 监听端口**：设置 WSS 端口，默认值为 10443。 
...
- **密钥密码**：密钥文件的密码即是您生成证书文件时生成的密码。如若没有，请将其保留为空。 

  *重要提示：默认情况下，WSS 证书已配置，不需要上传。默认证书文件是“**自签名**”证书，因 此在您使用浏览器打开 WebRTC 客户端时会弹出安全警告。您可以从第三方证书提供商购买证书，以避免安全警告。有关更多信息，请参考下一节 "[使用授权证书设置 WebRTC](使用授权证书设置 WebRTC)"。
...
通过单击“**WebRTC** > **通话**”菜单，您可快速查看 WebRTC 上的所有活跃通话及其通话详情。



### WebRTC 客户端

您可单击“**WebRTC > WebRTC 客户端**”，打开 PortSIP 内置 WebRTC 客户端，在网页发起和接听 WebRTC 呼叫。



#### 使用授权证书设置 WebRTC

您可以从 Thawte 或 Digicert 等证书提供商购买官方证书，以避免安全警报和手动添加安全例外。购买证书时，您需要自行生成专用密钥和 CSR，具体请阅读证书提供商的指示信息或联系提供商 支持。我们建议您不要设置专用密钥文件密码，并且您必须自行保存专用密钥文件。

假设您已从 Thawte 购买证书（在此我们以 Thawte SSL123 证书为例），用于您的 WebRTC 域名 *example.com*。在下载 certificate .zip 文件后，您需要将其解压缩，并获得两个文件： *IntermediateCA.crt* 和 *ssl_certificate.crt*。您需要使用纯文本编辑器（例如 Windows 记事本，请勿使用 MS Word）将两个证书合并为一个，将 *IntermediateCA.crt* 的所有文字附加到 *ssl_certificate.crt* 的文字后方。 
...
现在，您可从“**WebRTC**”菜单单击“**WebRTC 客户端**”以打开 WebRTC 客户端，输入您的分机号和密码以及 SIP 域名（分机的 SIP 域名，而非网关域名），按下“**登录**”按钮以登录至 WebRTC 客户端，然后发起或接听通话。 
...
PortSIP 包含 Session Border Controller (SBC)，可用于以下场景：

- 解决远程和 PBX 本地的 NAT 穿透问题。
- 简化远程和 PBX 本地的防火墙配置。
- 解决根据端口号阻止 VoIP 流量的 ISP 问题
- 允许在部分受限制的位置（例如酒店房间）使用 VoIP-over-WiFi。
- “**修复**”防火墙无法正确处理 VoIP 流量或存在问题而无法正确配置的问题，例如 Microsoft ISA Server。
...
SBC 功能当前仅支持 UDP 传输协议。

启用 SBC 后且指定了 SIP 信令和 STUN 端口后，您可关闭防火墙中的全部其他端口，仅开启 SIP 信令端口、STUN 端口和 STUN 端口 + 1，因为客户端、IP 话机和应用仅需要这三个端口来发起和接听呼叫。

**注意**：STUN 端口 +1 是指，当 STUN 端口为 3478 时，STUN 端口 + 1 即为 3479。



### 11.3 CTI功能

从 v12.2 版开始，PortSIP PBX 支持高级 CTI 功能，比如监听，密语，强插，强拆等。
...
通过设置 SMS，PortSIP PBX 能够在从分机收到短信时发送 SMS 消息。

要设置该功能，请转至管理控制台的“**个人资料** > **SMS**”。要启用 SMS 功能，应提供以下选项值：
...
单击左侧菜单的“**API 密码 (MD5)**”。在显示的“**更改 API 密码**”版块，输入您的账号密码和新 API 密码，单击“**更改 API 密码**”。该页面随即显示“**MD5 API 密码**”，请复制该 MD5 字符 串。 

现在，在 PortSIP PBX 中，请选择 SMS 提供商“**SMSAPI**”，将复制的 API MD5 字符串粘贴到“**API 密钥/令牌**”，为“**用户名**”输入您的 SMSAPI 账号。



**Nextmo** 

Nextmo 是一个 SMS 提供商，您可通过访问 https://www.nextmo.com注册账号并登录。
...
要配合使用 Twilio，您还需要购买一个号码以发送 SMS。请单击“**电话号码**”版块的“**管理号码**”，根据提示信息购买号码。 
...
**注意**，在将电话号码复制到 PortSIP PBX 前，请移除“(”、“-”和空格。例如，(305) 6768541 应更改为 3056768541。

重要提示：在将 SMS 提供商信息填写至 PortSIP PBX 前，建议您在 SMS 提供商管理控制台/面板发送一些测试信息，确保 SMS 正常运行。

**示例：**

如果您已设置 SMS提供商，现在分机需要向 PortSIP PBX 发送一条寻呼信息，并指示该寻呼信息是一条 SMS 信息。
...
论坛：https://forum.portsip.com

Skype ID：portsip，用户名为 support@portsip
...
### 16.2 在局域网中部署 PortSIP PBX 并注册到公网上的 SIP TRUNK
...
### 16.3 在启用 SBC 的情况下部署 PortSIP PBX

如果您的 PortSIP PBX 服务器具有静态公共 IP，并且不希望将网络拓扑结构暴露出来，则我们可以启用 PortSIP PBX 内置的 SBC。

请按照以下步骤配置 SBC：

- 运行“**设置向导**”。在第 1 步输入私有 IPv4 和公共 IPv4。
- 单击菜单“**高级** > **SBC**”，选择“**启用 SBC**”，输入要用的“**SIP 信令端口**”和“**STUN 端口**”，或保留默认端口。
- 单击“**确定**”按钮。

配置完 SBC 后，您可设置防火墙，禁用除 “**SIP 信令端口**”，“**STUN 端口**”和“**STUN 端口 + 1**”以外的所有端口。例如，您将“**SIP 信令端口**”设置为 5080，“**STUN 端口**”设置为 3478，则只需要在防火墙中打开 5080、3478 和 3479 端口，并且可关闭防火墙中的其他端口。

现在，您的客户端应用/IP 话机可向 5080 端口发送 SIP 消息，以注册到 PBX。
