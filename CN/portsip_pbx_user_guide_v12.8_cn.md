...
© 2023 博瞻信息技术有限公司。保留所有权利。未经博瞻信息技术有限公司的明确书面许可，不得出于任何目的、以任何形式或任何方式（无论是电子方式还是实体方式）对本手册内容的任何部分进行复制或传播。依据法律，复制包括将内容翻译为其他语言或格式。 
...
© 2023 博瞻信息技术有限公司。保留所有权利。 
博瞻信息 
湖南长沙市岳麓区麓天路 28 号五矿科技园 A2-907 
未经博瞻信息技术有限责任公司的明确书面许可，不得出于任何目的、以任何形式或任何方式（无论是电子方式还是实物方式）对本手册内容的任何部分进行复制或传播。依据法律，复制包括将内容翻译为其他语言或格式。 
...
- 在分机用户自定义选项中，可以通过设置 { "disable_welcome_email" : true} 来禁止 PBX 发送用户创建后的欢迎邮件。
- 在 WSI 和 分机 Webhook 事件中，新增 "extension_presence" 事件用来通知用户的状态改变信息。
- 在 WSI 和 分机 Webhook 的 CDR 事件中，CDR 信息新增 The "cdr_id" 字段用来标识 CDR。
- 修复如果用户在多个设备上同时登录，此时通过 REST API 发起呼叫会导致 CDR 错误的问题。
- 修复 INTERCOM 或者 PAGE 通话，60秒后会自动挂断的问题。
- 修复呼叫中心坐席在从 PBX 离线后，重新登录，状态时 READY，但是无法收到队列通话的问题。

#### V12.8.2 功能变更

- 修复 RTP 包头 Marker bit 标志和一些运营商的兼容问题。
- 修复在 IVR 转移通话的时候，没有回铃音的问题。
- 修复当存在多个假期的时候，假期没有生效的问题。
- 修复当假期日期跨年导致假期没有生效的问题。

#### V12.8.0 功能变更

- 修复一个 WebRTC 呼叫下一个潜在的崩溃问题
- 修正 Web 服务器的一个安全配置问题
- 修复一个可能丢失录音文件的问题
- 修复当仅分机用户启用录音，通话从 IVR/QUEUE/RING GROUP 转到分机，会导致录音失败的问题
- 性能上巨大的提升
...
1. Windows: 备份 `C:\ProgramData\PortSIP` 目录
2. Linux: 备份 `/var/lib/portsip` 目录
...
1. 按 [上述步骤](#备份) 备份 PBX，并将备份目录下载到本地。
2. 将 PortSIP PBX 安装到新机器上后，将刚下载的目录复制到 `c:\ProgramData\PortSIP\`(c:\ProgramData 是隐藏目录) 覆盖原目录。
...
1. 按 [上述步骤](#备份) 备份 PBX，并将备份目录下载到本地。
2. 将 PortSIP PBX 安装到新机器上后，将刚下载的目录复制到 `/var/lib/portsip` 覆盖原目录。
