...
- 自动设置队列成员为 "**准备接听"** 状态 - 如果勾选该选项，那么一旦成员注册到 PBX，状态即自动为就绪，以及一旦成员被振铃，不管有没有接听，停止振铃或者挂断通话后，状态也将自动设置为就绪状态；如果没有勾选，那么成员注册到 PBX 之后，状态为未就绪，成员需要调用REST API或者拨打指定的功能代码来将自己的状态设置为就绪。一旦成员被振铃，不管有没有接听，停止振铃或者挂断通话后，状态也将自动设置为未就绪状态。
...
我们可以在队列中，为某些特定号码的来电指定专门的客户坐席人员。一旦这些特定的号码来电，队列将优先将来电分配给指定的专门客户坐席人员，只有在这些专门的客户坐席人员正在通话或者没有就绪的时候，才会将来电分配给其他可用的坐席人员。
...
我们可以为所有的队列指定 VIIP 来电名单，一旦这些 VIP 号码来电，队列将会直接将其插入到队列最前，这样就可以被优先接听。VIP 名单是全局有效，作用于所有的队列。
...
我们可以为所有的队列指定骚扰号码名单。骚扰号码名单分为两级，一级骚扰名单和二级, 全局有效，作用域所有的队列。

如果来电号码在一级骚扰名单里，队列将播放指定的语音给呼叫者，呼叫者听到提示音后，按 1 键可以即可主动挂断通话，按 2 键将继续通话。

如果来电号码在二级骚扰名单里，队列将播放指定的语音给呼叫者，播完完成后队列将自动挂断通话。
