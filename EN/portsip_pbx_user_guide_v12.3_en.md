...
+ Redhat RHEL (64-bit)
+ CentOS 7.5 higher (64-bit), doesn't support 8.x 
...
### 2.4 Preparing for upgrading from v12.0 / v12.1

**If your currently installation is v12.2.x, please skip this 2.4 section.**



> **Warning:** since the trunk and inbound rule settings has been changed, please pay attention to below issues::
>
> + If you have the inbound rule in v12.0/12.1 which is assigned with more than one trunk/provider, once upgraded to v12.3, the rule will keep one trunk/provider for this inbound rule only.
>
> + You must check all inbound rules of all tenants. If the DID number mask for the inbound rule contains a range, wildcard or multiple numbers, you must replace it with a single number or a single \* before upgrading as inbound rule may only include single \* or a specific number in v12.3.
>
>   For example, if there is an inbound rule in v12.1 for which the DID number mask is 123;210-220;56\*; \*89, you must modify it to a single number as inbound rule may only include single \* or a specific number in v12.3.
...
Open new ports on firewall for v12.3.
...
Please perform below commands to upgrade (from v12.0 / v12.1 / v12.2.x).
