...
+ Redhat RHEL (64-bit)
+ CentOS 7.5 or higher (64-bit) 
+ Debian 9 or higher (64-bit)
...
Technical support is also available at [PortSIP Forum](https://forum.portsip.com/).
...
The free edition of PortSIP PBX offers a maximum of 3 simultaneous calls and unlimited extensions (users). If you require more simultaneous calls, please refer to [License Section](#11.9 License) for more details.
...
> > From v12, PortSIP PBX Linux edition is migrated to docker environment, which does not support RPM and Deb installer.
...
+ Below ports must be permitted by your firewall.
  ...
  UDP: 45000– 65000, 33000 -  34999
  TCP: 8899– 8900、8881、8882、8883、8884、8887、8888、9333、10443
  
  **Note:** if your server is a cloud server, you will also need open the ports on your cloud platform firewall rule for above ports.
...
#### Installing a fresh PortSIP PBX v12.2
...
After having successfully installed PortSIP PBX, you must setup the Linux Firewall Rules to enable PortSIP PBX working properly.
...
+ UDP ports: **45000– 65000, 33000 -  34999 **. These ports are used for the RTP sessions.
+ TCP: **8899– 8900, 8881, 8882, 8883, 8884,  8887, 8888, 9333,10443**. These ports are used for the Server control and WebRTC Gateway transport.
...
After successfully installed PortSIP PBX, you must setup the Windows Firewall Rules to enable PortSIP PBX working properly.
...
### 2.4 Upgrading from v12.0 / v12.1

> **Warning:** since the trunk and inbound rule settings has been changed, please pay attention to below issues::
>
> + If you have the inbound rule in v12.0/12.1 which is assigned with more than one trunk/provider, once upgraded to v12.2, the rule will keep one trunk/provider for this inbound rule only.
>
> + You must check all inbound rules of all tenants. If the DID number mask for the inbound rule contains a range, wildcard or multiple numbers, you must replace it with a single number or a single \* before upgrading as inbound rule may only include single \* or a specific number in v12.2.
>
>   For example, if there is an inbound rule in v12.1 for which the DID number mask is 123;210-220;56\*; \*89, you must modify it to a single number as inbound rule may only include single \* or a specific number in v12.2.



Before upgrade to v12.2, we recommend you backup your currently v12.0/v12.1 PBX:
...
+ If failed to upgrade the PBX, you can restore your PBX from the saved backup file, please see the [section 11.8](#11.8 Backup and Restore).



#### **Windows**

After downloaded the v12.2 installer for Windows, you only need to double-click the installer, which will guide you through the upgrade process.
...
> **Note:**
>
> + The **IP_ADDRESS** is the IP address of your PBX server. In this case it is 66.175.222.20, you will need to change it by yourself.
...
> + if you want the PortSIP PBX to restart automatically if the Linux is restarted, please add **--restart=always** parameter in to below command.
...
**Now open new ports for v12.2:**

CentOS: 

```
$ firewall-cmd --permanent --service=portsip-pbx --add-port=8882-8884/tcp
$ firewall-cmd --reload
...
### 2.5 Avoiding HTTPS Certificate Security Warnings
...
+ Go to Thawte or Versign or other certificate providers to purchase a SSL certificate. Save the private key as **portsip.key**
...
+ Now you can sign in PortSIP PBX Management Console by URL https://yourpbx.com:8887
...
PortSIP PBX listens on 8888 port for providing HTTP portal to access the PBX Management Console, and listens on 8887 port for providing HTTPS portal. More details please read the [2.4 section](#2.4 Avoid HTTPS Certificate Security Warnings). 

+ Access the PBX Management Console by visiting http://pbxserverip:8888. (Note, don't use the IE browser)
...
After setting up the domain (in this case it is portsip.io), the extension SIP URI will be sip:xxx@portsip.io. For example, the extension 101 SIP address would be: sip:101@portsip.io. If you don't want to use domain, enter the private IP (for example: 192.168.0.16) of the PC/Server which has installed the PortSIP PBX instead of the domain (FQDN). In this case the extension 101 SIP address would be: sip:101@192.168.0.16.
...
**Step 3:**
 ...
 You can set transport layer protocol for the SIP here, with the default transport UDP on port 5060.
...
Setup mail server. You may setup the mail server in this step for receiving notifications, voicemails, conference invitations and CDR downloads. You can use your SMTP server or Gmail SMTP server.
...
PortSIP PBX can be deployed on popular cloud platforms such as AWS, GCE, AZURE, Digital Ocean, Alibaba Cloud, UCloud, Tencent Cloud.
...
+ You have to generate the certificate files by yourself if you have not purchased certificates from a third-party certificate provider. Please download the certificate file tool from [PortSIP website](https://www.portsip.com/downloads/apps/PortCertMaker.exe) (or run *PortCertMaker.exe* in the installation path of PBX), enter your SIP domain. Once clicked “**Generate**” button, certificate files will be generated automatically.

+ The certificates include three files (assume your SIP domain is portsip.io):
  domain_key_portsip.io.pem
  domain_cert_portsip.io.pem
  root_cert_portsip.io.pem

  You can also follow below steps if you would like to purchase certificate files from a third-party provider (assume purchased certificate for portsip.io):
  ...
  + Rename the private key file as domain_key_portsip.io.pem
  ...
  + Use a plain text editor for example Windows Notepad (do not use MS Word) to open the Intermediate CA file and SSL certificate file, copy the Intermediate CA contents to append to the SSL certificate file, and rename SSL certificate file as domain_cert_portsip.io.pem
  + Download the root certificate from your SSL provider and rename it as root_cert_portsip.io.pem
...
+ Click the Upload button to choose the certificate files that you have generated for uploading, “**domain_cert_portsip.io.pem**” for the “**Certificate file**”, “**domain_key_portsip.io.pem**” for the “**Private key file**”, and “**root_cert_portsip.io.pem**” for the “**Root certificate file**”.

+ Enter the “**Certificate Private Key Password**”. This password is the one that you entered when generating the certificate files in previous steps. Leave it blank if you don’t have it
...
PortSIP PBX has inbuilt WebRTC Gateway which provides an intelligent bridge between PBX and the WebRTC client from webpage.
...
+ **Listen WSS**:  Once “Listen WSS” is selected, you should also fill in below fields: 

+ **Listen WSS on port**: Set a port for the WSS, default is 10443. 

+ **Gateway Domain Name**: WebRTC Gateway domain name, which must be resolvable. If you don’t know it, enter the PortSIP PBX IP. **Note:** by default the domain is already configured, which does not require modification. 
...
+ **Password of Private key**: The password of private key file is the one you entered when you generate the certificate file. If there is not one, please leave it blank. 
...
By click "**WebRTC > Call Sessions**" menu you can quickly monitor all the active calls and call details on the WebRTC.



### WebRTC Client

You can open the PortSIP inbuilt WebRTC client by click "**WebRTC > WebRTC Client**" menu in order to make & receive the WebRTC call in web page.



#### Setup WebRTC with Authorized certificates

You can purchase an official certificate from the certificate provider such as [Thawte](https://www.thawte.com/) or [Digicert](https://www.digicert.com/) to avoid security alerts and the necessity of adding security exceptions manually. When purchasing the certificate, you should generate the private key and CSR by yourself – please read the certificate provider’s instructions or ask the provider for support. We recommend you not to set the password for private key file, and you must keep the private key file by yourself.

Assume you have purchased a certificate from [Thawte](https://www.digicert.com/) (in this case we use the Thawte SSL123 certificate as example) for your WebRTC Domain example.com. After download the certificate .zip file, you need to extract it and will get two files: IntermediateCA.crt and ssl_certificate.crt. You need to use a plain text editor (for example Windows Notepad; do not use MS Word) to combine your two certs into one file by copying all text from IntermediateCA.crt and appending to ssl_certificate.crt, with the ssl_certificate.crt at the top and IntermediateCA.crt at the bottom. 
...
Now you can click the “WebRTC Client” from the “WebRTC” menu to open the WebRTC Client, enter your extension number and password, and the SIP domain (this is the SIP domain of extension, not Gateway domain name”),  click “**Login**” button to sign in the WebRTC Client to make & receive calls.
...
In v12.2 now the PortSIP PBX support advanced CTI features, for example, silence monitoring, whisper, barge-in, barge-break.
...
PortSIP PBX v12.2 allows set the outbound caller ID when make outbound call - just add a SIP header into the INVITE message.
...
PortSIP provides extraordinary technical support and service for all users, please contact us when you encountered any problems:

Forum: https://forum.portsip.com

Skype ID: portsip, the name is support@portsip
